...
#+end_src
Rust suggets that we can borrow the value instead, which is the same as moving a
reference to that value. This makes sence, since the =Vector= type should not
